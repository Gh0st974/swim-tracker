    ...
    <>
      ...
      {isOpen && (
        <div
          ...
          onClick={onClose}   // ← ferme la sidebar sans changer de page
        />
      )}
      ...
      <aside
        ...
        className={`
          ...
          ${isOpen
            ? 'w-60'                          // ouverte : pleine largeur
            : 'w-0 md:w-16'                   // fermée  : cachée mobile, icônes desktop
          }
        `}
      >
        {/* ── Nav items ── */}
        <nav className="flex-1 py-4 flex flex-col gap-1 px-2">
          {navItems.map(({ id, label, icon: Icon }) => {
            ...
            return (
              <button
                key={id}
                onClick={() => onNavigate(id)}   // App.jsx ferme la sidebar sur mobile
        ...
        <div className="px-3 py-4 border-t border-white/10">
          <div className={`flex items-center gap-2 transition-opacity duration-200 ${isOpen ? 'opacity-100' : 'opacity-0'}`}>
